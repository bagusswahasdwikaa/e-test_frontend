...
export default function DaftarUjianPage() {
  ...
  // fungsi untuk hitung status otomatis
  ...
  // update status otomatis tiap 1 menit
  useEffect(() => {
          ...
          <table className="min-w-full text-sm text-gray-800 table-fixed">
            <thead className="bg-blue-900 text-white text-center">
              <tr>
                ...
                <th className="px-4 py-3" style={{ width: 150 }}>
                  Tanggal Mulai
                </th>
                <th className="px-4 py-3" style={{ width: 150 }}>
                  Tanggal Akhir
                ...
                <th className="px-4 py-3" style={{ width: 110 }}>
                  Butir Soal
                </th>
                <th className="px-4 py-3" style={{ width: 110 }}>
                  Status
                </th>
                <th className="px-4 py-3" style={{ width: 230 }}>
                  Aksi
                </th>
              </tr>
            </thead>
                    ...
                    <td className="px-4 py-2">
                      <div className="flex justify-center items-center gap-2 flex-wrap">
                        <button
                          ...
                          onClick={() => router.push(`/admin/daftarUjian/lihatSoal?ujian_id=${ujian.id}`)}
                          type="button"
                        >
                          Lihat
                        </button>
                        <button
                          ...
                          onClick={() => router.push(`/admin/daftarUjian/editUjian?ujian_id=${ujian.id}`)}
                          type="button"
                        >
                          Edit
                        </button>
                        ...
                        <button
                          className="bg-green-700 hover:bg-green-800 text-white px-3 py-1 rounded text-xs flex items-center gap-1 cursor-pointer"
      ...
      {showModal && selectedUjian && (
        <div className="fixed inset-0 bg-gray-800 bg-opacity-60 flex items-center justify-center z-50">
          <div className="bg-gray-100 rounded-lg shadow-md p-6 w-full max-w-md mx-4 border border-gray-300">
            ...
            <div className="max-h-64 overflow-y-auto mb-6 border rounded-md p-3 bg-white">
              {pesertaList.length > 0 ? (
                pesertaList.map((peserta) => (
                  <label
                    key={peserta.ID_Peserta}
                    className="flex items-center gap-3 py-2 cursor-pointer hover:bg-blue-100 rounded-md px-2"
                  >
                    <input
                      type="checkbox"
